# app/account/routes.py
from flask import Blueprint, render_template, request, redirect, session, flash, current_app, jsonify
from config import SIDEBAR_CONFIG
from datetime import datetime
import pymysql
from .dao.user_dao import UserDao
from app.account.utils.mail import send_email
from app.mypage.dao.point_dao import PointDao
import random, string

bp = Blueprint(
    'account',
    __name__,
    template_folder='templates',
    static_folder='static',
    static_url_path='/account/static'
)

# ---------------------------------------------------------
# 전역 포인트 DAO (단 한 번만 생성) ← 가장 중요
# ---------------------------------------------------------
point_dao = PointDao(lambda: current_app.get_db_connection())


# ---------------------------------------------------------
# 로그인
# ---------------------------------------------------------
@bp.route("/login", methods=["POST"])
def login():
    userid = request.form.get("username")
    password = request.form.get("password")

    conn = current_app.get_db_connection()
    cursor = conn.cursor(pymysql.cursors.DictCursor)

    try:
        # 1. 사용자 조회
        cursor.execute("SELECT * FROM user WHERE id=%s AND password=%s", (userid, password))
        user = cursor.fetchone()

        if not user:
            return "<script>alert('아이디 또는 비밀번호가 틀렸습니다.'); history.back();</script>"

        if user.get("withdraw") == 1:
            return "<script>alert('탈퇴한 회원입니다.'); history.back();</script>"

        # 2. last_login_at 처리
        now = datetime.now()
        today = now.date()
        last_login_raw = user.get("last_login_at")

        if isinstance(last_login_raw, str):
            try:
                last_login = datetime.strptime(last_login_raw, "%Y-%m-%d %H:%M:%S")
            except:
                last_login = None
        else:
            last_login = last_login_raw

        is_first_today = (last_login is None) or (last_login.date() < today)

        # 3. 세션 저장
        user.pop("password", None)
        session["user"] = user

        # 4. 로그인 시간 업데이트
        cursor.execute("UPDATE user SET last_login_at=%s WHERE id=%s", (now, userid))
        conn.commit()

    finally:
        cursor.close()
        conn.close()

    # ---------------------------------------------------------
    # ⚡ DB 연결을 닫은 후 포인트 지급 → 빠르고 안전함
    # ---------------------------------------------------------
    if is_first_today:
        point_dao.add_point(userid, 100, "첫 로그인 보너스")

    return redirect("/")


# ---------------------------------------------------------
# 로그아웃
# ---------------------------------------------------------
@bp.route("/logout")
def logout():
    session.pop("user", None)
    flash("로그아웃 되었습니다.")
    return redirect("/")


# ---------------------------------------------------------
# 회원가입
# ---------------------------------------------------------
@bp.route('/join-agree')
def join_agree():
    return render_template("join_agree.html", sidebar=SIDEBAR_CONFIG["default"])


@bp.route('/join-find')
def join_find():
    return render_template("join_find.html", sidebar=SIDEBAR_CONFIG["default"])


@bp.route('/step2', methods=['POST'])
def process_agree():
    terms = request.form.get('agree-terms')
    privacy = request.form.get('agree-privacy')

    if not terms or not privacy:
        flash("필수 약관에 동의해야 합니다.", "error")
        return redirect('/join-agree')

    session['join_agreement'] = {
        "agree-terms": terms,
        "agree-privacy": privacy,
        "agree-marketing": request.form.get("agree-marketing", "off")
    }

    return redirect('/join-info')


@bp.route('/join-info')
def join_info():
    return render_template("join_info.html", sidebar=SIDEBAR_CONFIG["default"])


# ---------------------------------------------------------
# 회원가입 처리
# ---------------------------------------------------------
@bp.route('/join', methods=['POST'])
def join():
    userid = request.form.get("userid")
    password = request.form.get("password")
    confirm = request.form.get("confirm-password")
    name = request.form.get("name")
    nickname = request.form.get("nickname")
    email = request.form.get("email")

    interests = request.form.getlist("interests[]")
    skills = request.form.getlist("skills[]")

    if password != confirm:
        flash("비밀번호가 일치하지 않습니다.", "error")
        return redirect("/join-info")

    user_dao = UserDao()
    ok = user_dao.insert_user(
        {
            "id": userid,
            "password": password,
            "name": name,
            "nick": nickname,
            "email": email
        },
        interests=interests,
        skills=skills
    )

    if not ok:
        flash("회원가입 중 오류가 발생했습니다.", "error")
        return redirect("/join-info")

    # 가입 보너스 지급
    point_dao.add_point(userid, 1000, "회원가입 보너스")

    flash("회원가입 완료! 1000포인트 지급되었습니다.", "success")
    return redirect("/")


# ---------------------------------------------------------
# 중복 체크
# ---------------------------------------------------------
@bp.route("/check-duplicate", methods=["POST"])
def check_duplicate():
    field = request.form.get("field")
    value = request.form.get("value")

    if not field or not value:
        return {"status": "error", "message": "값이 없습니다."}

    conn = current_app.get_db_connection()
    cursor = conn.cursor(pymysql.cursors.DictCursor)

    try:
        sql_map = {
            "userid": "SELECT id FROM user WHERE id=%s",
            "nickname": "SELECT nick FROM user WHERE nick=%s",
            "email": "SELECT email FROM user WHERE email=%s"
        }

        sql = sql_map.get(field)
        if not sql:
            return {"status": "error", "message": "잘못된 필드입니다."}

        cursor.execute(sql, (value,))
        exists = cursor.fetchone()

        if exists:
            return {"status": "exists", "message": "이미 사용중입니다."}
        else:
            return {"status": "ok", "message": "사용 가능합니다."}

    finally:
        cursor.close()
        conn.close()


# ---------------------------------------------------------
# 아이디 찾기 / 비밀번호 찾기 (원본 그대로 유지)
# ---------------------------------------------------------
@bp.route('/send-id-code', methods=['POST'])
def send_id_code():
    name = request.form.get('name')
    email = request.form.get('email')

    user = UserDao.get_user_by_name_email(name, email)
    if not user:
        return jsonify(success=False, msg="일치하는 회원이 없습니다.")

    code = ''.join(random.choices(string.digits, k=6))

    session['find_id_code'] = code
    session['find_id_userid'] = user['id']

    send_email(email, "아이디 찾기 인증코드", f"인증코드: {code}")

    return jsonify(success=True, msg="인증코드를 이메일로 전송했습니다.")


@bp.route('/find-id', methods=['POST'])
def find_id():
    name = request.form.get('name-id')
    email = request.form.get('email-id')
    code = request.form.get('code-id')

    if code != session.get('find_id_code'):
        return "<script>alert('인증코드가 일치하지 않습니다.'); history.back();</script>"

    userid = session.get('find_id_userid')

    return f"""
    <script>
    alert("{name}님의 ID는 '{userid}' 입니다.");
    window.location.href='/join-find';
    </script>
    """


@bp.route('/find-password', methods=['POST'])
def find_password():
    userid = request.form.get('userid-pw')
    name = request.form.get('name-pw')
    email = request.form.get('email-pw')

    user = UserDao.get_user_by_all(userid, name, email)
    if not user:
        return "<script>alert('입력 정보가 일치하지 않습니다.'); history.back();</script>"

    temp_pw = ''.join(random.choices(string.ascii_letters + string.digits, k=10))

    UserDao.update_password(userid, temp_pw)
    send_email(email, "임시 비밀번호 안내", f"임시 비밀번호: {temp_pw}")

    return """
    <script>
    alert("임시 비밀번호를 이메일로 전송했습니다.");
    window.location.href='/join-find';
    </script>
    """
